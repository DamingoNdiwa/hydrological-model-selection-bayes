...
def run_analysis(params):
    ...
    model_prior_params = {
        "n":3,
        "k": {"loc": jnp.log(jnp.array([0.8, 0.2, 0.2])),
              "scale": jnp.array([0.25, 0.25, 0.25])},
        "k_int": {"loc": jnp.array([0.6, 0.6]),
    ...
    prior_predictive = dist.sample(num_results, seed=subkey)
    jnp.save('prior_obs',prior_predictive[-1], allow_pickle=True)
    ...
    post_save.to_csv('post_3reabucs.csv', index=False)
    ...
    df2.to_csv('meanlogll2bucs.csv', index=False)
    ...
    jnp.save('ppc3buc06', posterior_predictive[-1], allow_pickle=True)

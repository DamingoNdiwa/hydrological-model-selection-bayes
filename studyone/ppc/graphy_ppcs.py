...
# Import synthetic discharge for M2
y_obs1 = jnp.load('./data_1980.npy', allow_pickle=True)
...
M31_mu = jnp.mean(M31, axis=0)
M41_mu = jnp.mean(M41, axis=0)
...
ax1.plot(y_obs1[0], color='r', lw=0.5, alpha=0.5, ls='-')
ax1.plot(jnp.squeeze(M21_mu), color='b', lw=0.8, ls='--')
ax1.plot(jnp.squeeze(M31_mu), color='k', lw=0.8, alpha=0.8, ls='-.')
ax1.plot(jnp.squeeze(M41_mu), color='m', alpha=0.8, ls=':')
...
prior_ppc = np.count_nonzero((priorchains >= acf_yobs.reshape(91,
                                                              1) / M2.shape[0] * M2.shape[1]) <= (chains >= acf_yobs.reshape(91,
                                                                                                                             1)) / (M2.shape[0] * M2.shape[1])) / (M2.shape[0] * M2.shape[1])
print(f'the prior-ppp-value for model M2 is: {prior_ppc}')  # 0.564

# Plot of Autocor
fig = plt.figure()
ax = fig.add_subplot(111)
ax.plot(jnp.mean(chains, 1), acf_yobs, '.')
ax.text(-0.02, 0.78, 'posterior predictive $p$-value = 0.505', fontsize=7)
ax.text(
    -0.02,
    0.68,
    'prior calibrated posterior predictive $p$-value = 0.425',
    fontsize=7)

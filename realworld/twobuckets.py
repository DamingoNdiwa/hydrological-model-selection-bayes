...
def run_analysis(params):
    ...
    assert not np.any(np.isnan(df['observed_discharge']))
    ...
    model_prior_params = {
        "n":2,
        "k": {"loc": jnp.array([0.0,0.0]),
              "scale": jnp.array([1.0, 1.0])},
        "k_int": {"loc": jnp.array([0.0]),
                  "scale": jnp.array([1.0])},
    ...
    post_save.to_csv('post2_2bucsrea.csv', index=False)
    ...
    df2.to_csv('meanlogll2bucsrea10.csv', index=False)

...
#key, subkey = jax.random.split(key)
print(subkey)
*_,posterior_predictive = dist.sample(seed=key, k = jnp.array([0.834, 0.605, 0.576]), k_int = jnp.array([1.699, 0.726]), v_init = jnp.array([1.283,1.211,1.408]), v_max = jnp.array([0.957]), sigma = jnp.array([6.994]))
posterior_predictive = (jnp.squeeze(posterior_predictive))

# save the predicted dicharge values for post processing
jnp.save('ppcs1569bucsrea',posterior_predictive,allow_pickle=True)

#key, subkey = jax.random.split(subkey)
*_,posterior_predictive = dist.sample(seed=key, value=posterior_samples)
posterior_predictive = (jnp.squeeze(posterior_predictive))
jnp.save('ppcs31569sbucsrea80',posterior_predictive,allow_pickle=True)
print(subkey)
...
posterior_predictive = (jnp.squeeze(posterior_predictive))
jnp.save('ppcs3randomsbucsrea80',posterior_predictive,allow_pickle=True)
print(subkey)


#*_,posterior_predictive = dist.sample(seed=PRNGKey(0),  k = jnp.array([0.834, 0.605, 0.576]), k_int = jnp.array([1.699, 0.726]), v_init = jnp.array([1.283,1.211,1.408]), v_max = jnp.array([0.957]), sigma = jnp.array([6.994]))
#posterior_predictive = (jnp.squeeze(posterior_predictive))
#jnp.save('ppcs30bucsrea',posterior_predictive,allow_pickle=True)

#*_,posterior_predictive = dist.sample(seed=PRNGKey(1),  k = jnp.array([0.834, 0.605, 0.576]), k_int = jnp.array([1.699, 0.726]), v_init = jnp.array([1.283,1.211,1.408]), v_max = jnp.array([0.957]), sigma = jnp.array([6.994]))
#posterior_predictive = (jnp.squeeze(posterior_predictive))
#jnp.save('ppcs31bucsrea',posterior_predictive,allow_pickle=True)

#*_,posterior_predictive = dist.sample(seed=PRNGKey(3),  k = jnp.array([0.834, 0.605, 0.576]), k_int = jnp.array([1.699, 0.726]), v_init = jnp.array([1.283,1.211,1.408]), v_max = jnp.array([0.957]), sigma = jnp.array([6.994]))
#posterior_predictive = (jnp.squeeze(posterior_predictive))
#jnp.save('ppcs32bucsrea',posterior_predictive,allow_pickle=True)


#*_,posterior_predictive = dist.sample(seed=PRNGKey(5),  k = jnp.array([0.834, 0.605, 0.576]), k_int = jnp.array([1.699, 0.726]), v_init = jnp.array([1.283,1.211,1.408]), v_max = jnp.array([0.957]), sigma = jnp.array([6.994]))
#posterior_predictive = (jnp.squeeze(posterior_predictive))
#jnp.save('ppcs35bucsrea',posterior_predictive,allow_pickle=True)
